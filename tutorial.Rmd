...
tidy_new <- left_join(tidy, tidy_new , by = "Month")
  tidy_new
...
auto_fit <- lm(month_count~Month, tidy_new)
...
tidy_new2 <- left_join(tidy, tidy_new2 , by = "hour")
  tidy_new2

```
```{r reg4}

auto_fit2 <- lm(hour_count~hour, tidy_new2)
